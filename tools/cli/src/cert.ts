...
const CA_ORG = 'Open-Agent Dev CA Self Signed Org';
const CA_NAME = 'Open-Agent Dev CA Self Signed CN';
...
export class CertCommand extends Command {
  ...
  domain = Option.String('--domain', {
    description:
      'Generate certificates for given domain. e.g. "open-agent.localhost"',
  });
  ...
  private createCert(domain: string) {
    ...
    this.exec(
      `openssl req -new -key ${keyPath} -out ${csrPath} -config ${confPath} -subj "/C=/ST=/O=/localityName=/commonName=${domain}/organizationalUnitName=/emailAddress=${domain}@open-agent.io/"`
    );
    this.exec(
  ...
  private installCa() {
    ...
    this.exec(
      `openssl req -new -newkey rsa:2048 -days 1024 -nodes -x509 -subj "/C=/ST=/O=${CA_ORG}/localityName=/commonName=${CA_NAME}/organizationalUnitName=Developers/emailAddress=dev@open-agent.io/" -keyout ${CA_KEY_PATH} -out ${CA_PEM_PATH}`
    );
    this.trustCa(CA_PEM_PATH);
  }

  private trustCa(pem: Path) {
    this.logger.info(`Trusting Open-Agent Dev Self Signed CA`);
    this.exec(
