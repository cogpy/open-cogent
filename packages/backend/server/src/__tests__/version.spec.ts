...
test('should passthrough if version check is not enabled', async t => {
  ...
  res = await app.GET('/guarded/test').set('x-oa-version', '0.20.0');

  t.is(res.status, 200);

  res = await app.GET('/guarded/test').set('x-oa-version', 'invalid');
...
test('should passthrough is version range is invalid', async t => {
  ...
  let res = await app.GET('/guarded/test').set('x-oa-version', 'invalid');

  t.is(res.status, 200);
});

test('should pass if client version is allowed', async t => {
  let res = await app.GET('/guarded/test').set('x-oa-version', '0.20.0');

  t.is(res.status, 200);

  res = await app.GET('/guarded/test').set('x-oa-version', '0.21.0');
  ...
  res = await app.GET('/guarded/test').set('x-oa-version', '0.19.0');
...
test('should tell downgrade if client version is higher than allowed', async t => {
  ...
  let res = await app.GET('/guarded/test').set('x-oa-version', '0.23.0');
...
test('should test prerelease version', async t => {
  ...
  let res = await app
    .GET('/guarded/test')
    .set('x-oa-version', '0.19.0-canary.1');

  // 0.19.0-canary.1 is lower than 0.19.0 obviously
  t.is(res.status, 403);

  res = await app.GET('/guarded/test').set('x-oa-version', '0.20.0-canary.1');

  t.is(res.status, 200);

  res = await app.GET('/guarded/test').set('x-oa-version', '0.20.0-beta.2');
