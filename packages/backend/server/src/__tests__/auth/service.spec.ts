...
test('should be able to sign in by password', async t => {
  ...
  const signedInUser = await auth.signIn('u1@open-agent.io', '1');

  t.is(signedInUser.email, 'u1@open-agent.io');
});

test('should throw if user not found', async t => {
  const { auth } = t.context;

  await t.throwsAsync(() => auth.signIn('u2@open-agent.io', '1'), {
    message: 'Wrong user email or password: u2@open-agent.io',
  });
});
...
test('should throw if password not match', async t => {
  ...
  await t.throwsAsync(() => auth.signIn('u1@open-agent.io', '2'), {
    message: 'Wrong user email or password: u1@open-agent.io',
  });
});
...
test('should be able to change password', async t => {
  ...
  let signedInU1 = await auth.signIn('u1@open-agent.io', '1');
  t.is(signedInU1.email, u1.email);

  await auth.changePassword(u1.id, 'hello world affine');

  await t.throwsAsync(
    () => auth.signIn('u1@open-agent.io', '1' /* old password */),
    {
      message: 'Wrong user email or password: u1@open-agent.io',
    }
  );

  signedInU1 = await auth.signIn('u1@open-agent.io', 'hello world affine');
  t.is(signedInU1.email, u1.email);
});
...
test('should be able to change email', async t => {
  ...
  await auth.changeEmail(u1.id, 'u2@open-agent.io');

  await t.throwsAsync(
    () => auth.signIn('u1@open-agent.io' /* old email */, '1'),
    {
      message: 'Wrong user email or password: u1@open-agent.io',
    }
  );

  signedInU1 = await auth.signIn('u2@open-agent.io', '1');
  t.is(signedInU1.email, 'u2@open-agent.io');
});
...
test('should be able to sign in different user in a same session', async t => {
  ...
  const u2 = await auth.signUp('u2@open-agent.io', '1');
