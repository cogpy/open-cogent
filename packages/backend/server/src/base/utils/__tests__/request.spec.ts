...
test('should get client version from x-oa-version header', t => {
  const req = {
    headers: {
      'x-oa-version': '0.22.2',
    },
  ...
  const req2 = {
    headers: {
      'x-oa-version': ['0.22.2', '0.23.0-beta.2'],
    },
  } as unknown as Request;
...
test('should not get client version from x-oa-version header', t => {
  const req = {
    headers: {
      'user-agent':
